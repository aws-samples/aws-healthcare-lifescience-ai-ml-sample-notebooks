...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of devices for tensor sharding.",
    )
...
def get_model(debug=False):
    base_model = EsmForMaskedLM.from_pretrained("facebook/esm2_t6_8M_UR50D")
    ...
    my_model = EsmForMaskedLM(my_config)

    class ParallelSelfAttention(transformers.models.esm.modeling_esm.EsmSelfAttention):
        def __init__(self, config, position_embedding_type=None):
            super().__init__(config, position_embedding_type)

            self.query = layers.ColumnParallelLinear(
                config.hidden_size, self.all_head_size, gather_output=False
            )
            self.key = layers.ColumnParallelLinear(
                config.hidden_size, self.all_head_size, gather_output=False
            )
            self.value = layers.ColumnParallelLinear(
                config.hidden_size, self.all_head_size, gather_output=False
            )
    ...
    class ParallelSelfOutput(transformers.models.esm.modeling_esm.EsmSelfOutput):
    ...
    for layer in my_model.esm.encoder.layer:
